...
public class PathBAsed : MonoBehaviour
{
    public int CutCascades = 1;  // ← New line
    ...
    private List<Vector3> cuttingPath = new List<Vector3>();
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.CompareTag("Dagger"))
        {
            ...
            StartCutting(collision.contacts[0].point);
        }
    }
    ...
    {
       // Debug.Log("hitting");
        if (isCutting && collision.gameObject.CompareTag("Dagger") && Time.time - lastCutTime > CutInterval)
        {
            AddCutPoint(collision.contacts[0].point);
    ...
    void StartCutting(Vector3 initialPoint)
    {
        cuttingPath.Clear();
        cuttingPath.Add(initialPoint);
        isCutting = true;
        lastCutTime = Time.time;
    }

    void AddCutPoint(Vector3 point)
    {
        cuttingPath.Add(point);
        lastCutTime = Time.time;
    }
    ...
    void SliceAlongPath()
    {
        ...
        int cuts = Mathf.Min(MaxCuts, cuttingPath.Count - 1);
        for (int i = 0; i < cuts; i++)
        {
            Vector3 start = cuttingPath[i];
            Vector3 end = cuttingPath[i + 1];
            Vector3 direction = (end - start).normalized;
            
            // Improved plane orientation
            Vector3 planeNormal = Vector3.Cross(direction, Camera.main.transform.up).normalized;
            if (planeNormal == Vector3.zero)
                planeNormal = Vector3.Cross(direction, Camera.main.transform.forward).normalized;

            Plane cuttingPlane = new Plane(planeNormal, (start + end) * 0.5f);

            foreach (var part in parts)
            {
                subParts.Add(GenerateMesh(part, cuttingPlane, true));
                subParts.Add(GenerateMesh(part, cuttingPlane, false));
            }
        ...
        foreach (var part in parts)
        {
            ...
            part.GameObject.GetComponent<Rigidbody>().AddForceAtPosition(
                part.Bounds.center * ExplodeForce, 
                transform.position
            );
        }
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(PathBAsed original)
        {
            ...
            var meshDestroy = GameObject.AddComponent<PathBAsed>();
